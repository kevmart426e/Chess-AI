...
import hw2.agents.AlphaBetaAgent.AlphaBetaSearcher;
...
public class AlphaBetaAgent extends ChessAgent
{
	...
	public class AlphaBetaSearcher extends Object implements Callable<Pair<Move, Long> >
	{
		...
		{
			Double memoValue = memoTable.get(node);
			if (memoValue != null) {
		        node.setMaxPlayerUtilityValue(memoValue);
		        return node;
		    }
			else if(node.isTerminal()) // terminal state!
			{
				return node;
			} else if(depth <= 0) // reached the end of the depth!
			{
				...
				memoTable.put(node, node.getMaxPlayerUtilityValue());
				return node;
			} else // we can get the children of this node and find its best value
			{
				...
				memoTable.put(node, bestUtilityValue);
				return bestNode;
			}
		}
		...
		public boolean is_quiescence(DFSTreeNode node) {
			//return true if capture move
			return node.getMove().getType() == MoveType.CAPTUREMOVE;
		}
	...
	public static Map<DFSTreeNode, Double> memoTable;
